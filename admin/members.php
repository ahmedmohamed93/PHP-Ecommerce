						...
						<?php
							foreach($rows as $row) {
								echo "<tr>";
									...
									echo "<td>";
									if(empty($row['Avatar'])){
										echo "<img src='upload/user_default.png' alt='' />";
									}else{
...
<?php
			...
			echo "<div class='container'>";
			    echo "<div class='nice-message'>There's Is No Users To Show</div>";
	...
	<?php 
		...
		<?php 
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                @$avatarExension = strtolower(end(explode('.', $avatarName)));
				...
				if(!empty($avatarName) && !in_array($avatarExension, $avatarAllowedExtension)){
                	$formErrors[] = ' This Extension Is Not <strong>Allowed</strong>'; 
                }
                if(empty($avatarName)){
                	$formErrors[] = 'You Must Upload Avatar'; 
                }
				...
				if (empty($formErrors)) {
					...
					if ($check == 1) {
						...
						$stmt = $con->prepare("INSERT INTO 
							                      users(Username, Password, Email, FullName,RegStatus, 
							                           Date,Avatar)
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				if (empty($formErrors)) {
					...
					if($count == 1){
						$theMsg=  '<div class="alert alert-danger">This Username ALeardy Exist</div>';
						redirectHome($theMsg, 'back');	
					}else{
